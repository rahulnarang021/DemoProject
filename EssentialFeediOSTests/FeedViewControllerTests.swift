...
final class FeedViewControllerTests: XCTestCase {
    ...
    func test_imageDoesnotLoadAfterCellFFGoesOffScreen() {
        let image0 = makeImage(url: URL(string: "http://url-0.com")!)
        let (sut, loader) = makeSUT()
        ...
        sut.simulateFeedImageViewNotVisible(at: 0, view: cell!)

        let imageData0 = UIImage.make(withColor: .red).pngData()!
        loader.completeImageLoading(with: imageData0, at: 0)
        
        XCTAssertNil(cell?.renderedImage, "Cell should not complete imageLoad after invisible")
        
        
    }
    
    func test_feedLoadCompletesInBackground_shouldUpdateView() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        let feed = [makeImage()]
        let exp = expectation(description: "Loading for feed completes")
    ...
    func test_feedImageLoadCompletesInBackground_shouldUpdateView() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        let feed = [makeImage()]
        loader.completeFeedLoading(with: feed, at: 0)
        ...
        let exp = expectation(description: "Loading image for feed completes")
        DispatchQueue.global().async {
            let imageData0 = UIImage.make(withColor: .red).pngData()!
    ...
    private func makeSUT(file: StaticString = #file, line: UInt = #line) -> (sut: FeedViewController, loader: LoaderSpy) {
        ...
        testMemorLeak(loader, file: file, line: line)
        testMemorLeak(sut, file: file, line: line)
        return (sut, loader)
    }
...
private extension FeedViewController {
    ...
    func simulateFeedImageViewNotVisible(at row: Int, view: UITableViewCell) {
        let delegate = tableView.delegate
        let index = IndexPath(row: row, section: feedImagesSection)
        delegate?.tableView?(tableView, didEndDisplaying: view, forRowAt: index)
    }
